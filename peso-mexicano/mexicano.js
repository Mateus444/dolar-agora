	...
	let url = "https://economia.awesomeapi.com.br/json/last/MXN-BRL";
	...
	jsonRetorno.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
      		...
      		let cotacao = retornoParaObject.MXNBRL.bid;
      		let varicao = retornoParaObject.MXNBRL.pctChange;
      		...
      		document.getElementById("entrada-2").value = cotacaoFloat.toFixed(2);
                 let data = retornoParaObject.MXNBRL.create_date;
...
let multi = [2,10,20,30,40,50,100,200,300,1000];
            let escrito = ["(Dois pesos)", "(Dez pesos)","(Vinte pesos)", "(Trinta pesos)","(Quarenta pesos)","(Cinquenta pesos)","(Cem pesos)","(Duzentos pesos)","(Trezentos pesos)", "(Mil pesos)"];
